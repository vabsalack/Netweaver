...
class PlotTraining:
    ...
    def live_plot(self, *, xtp_slips=(30, 15), loss_caplimit=1.5, acc_caplimit=1.1, lr_caplimit=0.06):
        ...
        if self.path_epoch_log is not None:
            (self.artist_eploss,) = self.axes_loss.plot([], [], label="Epoch Loss", color="blue", linewidth=3)
            (self.artist_epacc,) = self.axes_acc.plot([], [], label="Epoch Acc", color="orange", linewidth=3)
        ...
        if self.path_validation_log is not None:
            (self.artist_vloss,) = self.axes_loss.plot([], [], label="Val Loss", color="blue", alpha=0.4, linewidth=3)
            (self.artist_vacc,) = self.axes_acc.plot([], [], label="Val Acc", color="orange", alpha=0.4, linewidth=3)

        if self.path_batch_log is not None:
            (self.artist_lr,) = self.axes_lr.plot([], [], label="LR decay", color="grey", alpha=0.3, linestyle="-", linewidth=2)
    ...
    def static_plot(self):
        ...
        if self.path_batch_log is not None:
            ...
            if not data_batch.empty:
                (artist_lr,) = axes_lr.plot(
                    data_batch["xaxis_value"], data_batch["learning_rate"], label="LR decay", color="grey", alpha=0.3, linestyle="-", linewidth=2
                )
        ...
        if self.path_validation_log is not None:
            ...
            if not data_validation.empty:
                (artist_vloss,) = axes_loss.plot(
                    data_validation["epoch"], data_validation["loss_validation"], label="Val Loss", color="blue", alpha=0.4, linewidth=2
                )
                (artist_vacc,) = axes_acc.plot(
                    data_validation["epoch"], data_validation["accuracy_validation"], label="Val Acc", color="orange", alpha=0.4, linewidth=2
                )
